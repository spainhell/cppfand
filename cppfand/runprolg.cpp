...
void ChainLst(Chained* Root, Chained* NewOfs) // assembler
{
	// ES:DI = Root;
	// ...
}
...
TDatabase* FindDataBase(pstring S)
{
	...
	db = Roots->Databases;
	while (db->Chain != nullptr) {
		if (db->Name == S) goto label1;
		next = (TDatabase*)db->Chain;
	}
label1:
	return (TDatabase*)db->Chain;
}
...
TPTerm* RdPrimExpr(TDomain* D, integer Kind)
{
	...
	default: {
		if (!RdVar(D, Kind, -1, nullptr /*tofs*/) && !RdConst(D, tofs))
			if (IsKeyWord("length")) tofs = GetFunOp(D, IntDom, _length, 's', Kind);
			else if (IsKeyWord("pos")) tofs = GetFunOp(D, IntDom, _pos, "sl", Kind);
			else if (IsKeyWord("min")) tofs = GetFunOp(D, IntDom, _min, "ii", Kind);
			else if (IsKeyWord("max")) tofs = GetFunOp(D, IntDom, _max, "ii", Kind);
			else if (IsKeyWord("val")) tofs = GetFunOp(D, IntDom, _val, 's', Kind);
			else if (IsKeyWord("copy")) tofs = GetFunOp(D, StrDom, _copy, "sii", Kind);
			else if (IsKeyWord("str")) tofs = GetFunOp(D, StrDom, _str, 'i', Kind);
			else if (IsKeyWord("repeatstr")) tofs = GetFunOp(D, StrDom, _repeatstr, "si", Kind);
			else if (IsKeyWord("leadchar")) tofs = GetFunOp(D, StrDom, _leadchar, "cs", Kind);
			else if (IsKeyWord("trailchar")) tofs = GetFunOp(D, StrDom, _trailchar, "cs", Kind);
			else if (IsKeyWord("maxrow")) tofs = GetFunOp(D, IntDom, _maxrow, "", Kind);
			else if (IsKeyWord("maxcol")) tofs = GetFunOp(D, IntDom, _maxcol, "", Kind);
			else Error(511);
	}
	}
	return (TPTerm*)&tofs;
...
TDomain* GetDomain(bool Create, pstring Nm)
{
	...
	d = Roots->Domains;
	TDomain* dofs = (TDomain*)d->Chain; // absolute d 
	while ((dofs != nullptr) && (d->Name != Nm)) dofs = (TDomain*)d->Chain;
	if (dofs == nullptr)
		if (copy(Nm, 1, 2) == "L_") {
	...
	while (dofs != 0) {
		...
		case _FunD: {
			...
			while (fdofs != 0) {
				...
				fdofs = (TDomain*)fd->Chain;
			}
			break;
		}
		}
		dofs = (TDomain*)d->Chain;
	}
}
...
TPredicate* GetPredicate() /*PPredicate*/
{
	...
	while (pofs != nullptr) {
		if (LexWord == p->Name) { RdLex(); goto label1; }
		pofs = (TPredicate*)p->Chain;
	}
	pofs = ClausePreds;
	while (pofs != 0) {
		if (LexWord == p->Name) { RdLex(); goto label1; }
...
TCommand* RdCommand() /*PCommand*/
{
	...
	label22:
		...
		if (pofs == (TDatabase*)&MemPred) {
			...
			if (pofs == (TDatabase*)&AddPred) { RdTermList(c, d->ElemDom, 2); Accept(','); }
			else if (pofs == (TDatabase*)&DelPred) { RdTermList(c, d->ElemDom, 1); Accept(','); }
			RdTermList(c, dofs, 2);
			Accept(',');
		...
		if (CFile->Typ == 'X') {
			...
			while (k != nullptr) {
				...
				while (kf != nullptr) {
					...
					while (flofs != nullptr) {
						f = fl->FldD; flofs = (TFldList*)fl->Chain;
						if (f == kf->FldD) {
							w = w | m;
							a[i] = n;
...
WORD ReadProlog(WORD RecNr)
{
	...
	pstring Reell = "Real";
	TDatabase* db = nullptr;
	//WORD* dbofs = &db->Chain; // absolute db dbofs;
	TPredicate* p = nullptr;
	//WORD* pofs = &p->Chain;
	TDomain* d = nullptr;
	//WORD* dofs = &d->Chain;
	...
	d->Chain = BoolDom;
	f->Chain = new TFunDcl(); // GetZStor(sizeof(TFunDcl) - 3 * 2);
	f->Name = StorStr("false");
	ChainLst(d->FunDcl, f->Chain);
	f->Chain = new TFunDcl(); // GetZStor(sizeof(TFunDcl) - 3 * 2);
	f->Name = StorStr("true");
	ChainLst(d->FunDcl, f->Chain);
	p->Chain = new TPredicate(); // GetZStor(sizeof(TPredicate) - 6);
	p->Name = StorStr("main");
	Roots->Predicates = (TPredicate*)p->Chain;
	LexDom = MakeDomain(_FunD, "Lexem");
	d->Chain = LexDom;
	f->Chain = new TFunDcl(); // GetZStor(sizeof(TFunDcl));
	f->Name = StorStr("lex");
	f->Arity = 3;
	f->Arg[0] = IntDom; f->Arg[1] = IntDom; f->Arg[2] = StrDom;
	ChainLst(d->FunDcl, f->Chain);
	LLexDom = MakeDomain(_ListD, "L_Lexem");
	d->Chain = LLexDom; d->ElemDom = LexDom;
	...
	while (Lexem != 0x1A) {
		...
		if (IsKeyWord("DOMAINS")) RdDomains();
			...
			db->Chain = FindDataBase(s);
			if (db->Chain == nullptr) {
				db->Chain = new TDatabase(); // GetZStor(sizeof(TDatabase) - 1 + s.length());
				ChainLst(Roots->Databases, db->Chain);
	...
	db->Chain = Roots->Databases;
	while (db->Chain != nullptr) {
		//db->SOfs = WORD(OPtr(_Sg, SaveDb(dbofs, AA)));
		db = (TDatabase*)db->Chain;
	}
...
label1:
	if (b1->Chain != nullptr)
		if (b1->Chain = B) {
			b1->Chain = B->Chain;
			while ((Q != nullptr)) {
				if (Q->NextBranch == (void*)B) Q->NextBranch = (TBranch*)B->Chain;
				Q = Q->PrevInst;
			}
		}
		else { b1 = (TDbBranch*)b1->Chain; goto label1; }
...
label5:
	...
	while ((q != nullptr) && (q->NextBranch == nullptr) &&
		!((TCommand*)(ptr(_Sg, q->RetCmd))->Code in[_NotC, _AllC]))
	{
		q1 = q;
		q = q->PrevInst;
	}
	...
	if (q->NextBranch == nullptr) {
		q1 = q; q = q1->RetInst;
		b = ptr(_Sg, q1->RetBranch);
		cofs = q1->RetCmd;
		CurrInst = q;
		TopInst = q1->PrevInst;
		Mem2.Release(q1);
		if (c->Code == _NotC) {
			if (Trace()) { printf("FAIL" ^ m ^ j"RETURN not()"); WaitC(); }
		}
		else {
...
label6:
	if (Trace()) { printf("REDO ", (pstring*)(ptr(_Sg, p->Name))^); WaitC(); }
	goto label2;
