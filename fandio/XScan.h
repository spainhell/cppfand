#pragma once
#include "XWKey.h"
#include "../Common/LinkD.h"
#include "../Common/LocVar.h"

struct KeyInD;

enum class ScanMode : uint8_t
{
	Sequential = 0,      // Sequential scan (no index)
	Index = 1,           // Index scan (ordered by key)
	Interval = 2,        // Interval scan (key ranges)
	WorkingIndex = 3,    // Working index scan
	SQL = 4,             // SQL scan
	LocalVariable = 5    // Local variable record
};

class XScan
{
public:
	XScan(FileD* aFD, XKey* aKey, std::vector<KeyInD*>& aKIRoot, bool aWithT);
	~XScan();
	FileD* FD = nullptr;
	XKey* Key = nullptr;
	FrmlElem* Bool = nullptr;
	ScanMode Kind = ScanMode::Sequential;
	int NRecs = 0, IRec = 0, RecNr = 0;
	bool hasSQLFilter = false, eof = false;
	void Reset(FrmlElem* ABool, bool SQLFilter, Record* record, RunErrorCallback err_callback);
	void ResetSort(std::vector<KeyFldD*>& aSK, FrmlElem* BoolZ, LockMode OldMd, bool SQLFilter, Record* record, MsgCallback msg_callback, RunErrorCallback err_callback);
	void SubstWIndex(XWKey* WK);
	void ResetOwner(XString* XX, FrmlElem* aBool, RunErrorCallback err_callback);
	[[nodiscard]] int32_t ResetOwnerIndex(LinkD* LD, LocVar* LV, FrmlElem* aBool, RunErrorCallback err_callback);
#ifdef FandSQL
	void ResetSQLTxt(FrmlElem* Z);
#endif
	void ResetLV(Record* aRP);
	void Close();
	void SeekRec(int I);
	void GetRec(Record* record);
private:
	std::vector<KeyInD*> KIRoot;
	LocVar* OwnerLV = nullptr;
	std::vector<KeyFldD*> SK;
	size_t _item = 1;
	XPage* page_ = nullptr;
	unsigned short items_on_page_ = 0;
	std::vector<KeyInD*>::iterator KI;
	int NOfKI = 0, iOKey = 0;
	bool TempWX = false;
	bool NotFrst = false;
	bool withT = false;
	Record* Strm = nullptr; // {SQLStreamPtr or LVRecPtr}
	void SeekOnKI(int I);
	void SeekOnPage(int pageNr, unsigned short i);
	void NextIntvl();
};