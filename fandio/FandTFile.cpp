...
unsigned short RdPrefix()
{
	...
	case FileType::FAND8: {
		RdWrCache(READ, CFile->FF->Handle, not_cached, 0, 2, &X8.NRs);
		RdWrCache(READ, CFile->FF->Handle, not_cached, 2, 2, &X8.RLen);
		CFile->FF->NRecs = X8.NRs;
		if (CFile->FF->RecLen != X8.RLen) { return X8.RLen; }
		break;
	}
	case FileType::DBF: {
		RdWrCache(READ, CFile->FF->Handle, not_cached, 0, 1, &XD.Ver);
		RdWrCache(READ, CFile->FF->Handle, not_cached, 1, 1, &XD.Date[0]);
		RdWrCache(READ, CFile->FF->Handle, not_cached, 2, 1, &XD.Date[1]);
		RdWrCache(READ, CFile->FF->Handle, not_cached, 3, 1, &XD.Date[2]);
		RdWrCache(READ, CFile->FF->Handle, not_cached, 4, 4, &XD.NRecs);
		RdWrCache(READ, CFile->FF->Handle, not_cached, 8, 2, &XD.HdLen);
		RdWrCache(READ, CFile->FF->Handle, not_cached, 10, 2, &XD.RecLen);
		CFile->FF->NRecs = XD.NRecs;
		if ((CFile->FF->RecLen != XD.RecLen)) { return XD.RecLen; }
		CFile->FF->FrstDispl = XD.HdLen;
		break;
	}
	default: {
		RdWrCache(READ, CFile->FF->Handle, not_cached, 0, 4, &X6.NRs);
		RdWrCache(READ, CFile->FF->Handle, not_cached, 4, 2, &X6.RLen);
		CFile->FF->NRecs = abs(X6.NRs);
		if ((X6.NRs < 0) && (CFile->FF->file_type != FileType::INDEX) || (X6.NRs > 0) && (CFile->FF->file_type == FileType::INDEX)
			|| (CFile->FF->RecLen != X6.RLen)) {
			return X6.RLen;
		}
		break;
	}
...
void WrPrefix()
{
	...
	if (IsUpdHandle(CFile->FF->Handle))
	{
		...
		default: {
			Pfx6.RLen = CFile->FF->RecLen;
			if (CFile->FF->file_type == FileType::INDEX) Pfx6.NRs = -CFile->FF->NRecs;
			else Pfx6.NRs = CFile->FF->NRecs;
			RdWrCache(WRITE, CFile->FF->Handle, not_cached, 0, 4, &Pfx6.NRs);
			RdWrCache(WRITE, CFile->FF->Handle, not_cached, 4, 2, &Pfx6.RLen);
		}
		}
	}
}
...
void WrPrefixes()
{
	...
	if (CFile->FF->TF != nullptr && IsUpdHandle(CFile->FF->TF->Handle))
		CFile->FF->TF->WrPrefix();
	if (CFile->FF->file_type == FileType::INDEX && CFile->FF->XF->Handle != nullptr
		&& /*{ call from CopyDuplF }*/ (IsUpdHandle(CFile->FF->XF->Handle) || IsUpdHandle(CFile->FF->Handle)))
		CFile->FF->XF->WrPrefix();
}
