...
void UpdScreen()
{
	...
	label1:
		r++;
		if (bScroll && (T[index] == 0x0C)) { InsPage = InsPg; index++; }
	} while (r <= PageS);
}
...
void CleanFrameM()
{
	if (Mode == SinFM || Mode == DouFM || Mode == DelFM || Mode == NotFM) /* !!! with Event do!!! */
			...
			((Event.What == evKeyDown) && (Event.KeyCode == _ESC_)) || (Event.What != evKeyDown))
		{
			...
			UpdStatLine(LineL, Posi, Mode); return;
		}
}
	...
	label2:
		...
		if (s == nullptr)
			...
			if ((s->LL > 0) && (s->A[1] == '{')) {
				...
				if (s2->LL == 0) { if (delta == 0) /*goto label4*/ throw std::invalid_argument("Editor::Help - delta is equal 0"); }
