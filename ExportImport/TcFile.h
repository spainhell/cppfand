...
const uint8_t MaxMatchLen = 18;
const uint8_t MinMatchLen = 3;
const WORD Leer = RingBufSz;

struct TXBuf {
	uint8_t RingBuf[RingBufSz + MaxMatchLen - 1]{ 0 };
...
public:
	...
	uint8_t CodeBuf[17]{ 0 }; // [0]=8 flags:"1"=unencoded byte,"0"=position/length word
	TXBuf* XBuf = nullptr;
