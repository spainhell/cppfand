...
public:
	...
	TextEditorEvents();
	~TextEditorEvents();
	void HandleEvent(TextEditor* editor, EditorMode& mode, bool& IsWrScreen, uint8_t SysLColor, std::string& LastS, WORD LastNr, std::vector<EdExitD*>& ExitD, std::vector<WORD>& breakKeys);
...
private:
	...
	void Wr(std::string s, std::string& OrigS, EditorMode mode, uint8_t SysLColor);
	...
	bool MyGetEvent(TextEditor* editor, EditorMode& mode, uint8_t SysLColor, std::string& LastS, WORD LastNr, bool IsWrScreen, bool bScroll, std::vector<EdExitD*>& ExitD, std::vector<WORD>& breakKeys);
